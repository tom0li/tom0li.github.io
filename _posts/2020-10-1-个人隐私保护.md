...
5、**自定义控输入法**。  
6、**自定义浏览器**，浏览器配置(user.js)、浏览器插件、证书、自定义浏览器设置 (浏览器每次浏览器操作后清空cookie、Indexed DB、本地存储 等存储、历史记录、缓存、网站设置、搜索历史等记录；自定义搜索建议；自定义数据反馈等)。  
7、**自定义APP应用权限**  
...
13、**所有密码**使用多类字符20位以上，无规律，一个服务一个密码，达到自己记不住**不知道**的目标。  
14、每次忘记登录，更改密码。  
15、离线存储密码。  
16、设置BOIS密码，开锁密码。  
